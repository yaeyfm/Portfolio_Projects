----- SQL Project Clean & Explore Date----
...
---------********************************************************************************************************-------------------------------------------------------------------
	...
	--Data Grouped by hospital  -- Too Many hospitalas

	Select Hospital,
			Count(Distinct(CONCAT(Clean_Doctor, Hospital ))) as Unique_Doctor_Count,
			Count(Distinct(CONCAT(Clean_Name, Age, [Blood Type], Gender ))) as Unique_Patient_Count,
			AVG(Days_Admitted) as Avg_DaysAdmitted, Sum([Billing Amount]) as Total_Billing
	FROM Projects..Healthcare
	Group by Hospital
	Order by Total_Billing Desc;
...
-----------------------------------------Biling Analysis----------------------------------------
	...
	--Running Total Billed by Dischardged date for year 2020

		Select Clean_Discharge_Date, 
				Sum([Billing Amount]) OVER(Partition by Clean_Discharge_Date Order by Clean_Discharge_Date) as Running_Total
		FROM Projects..Healthcare
	
		Where Clean_Discharge_Date < '2021-01-01'
		Order by Clean_Discharge_Date
	

-----------------------------------------------------------------------------------------------------


		Select YEAR(Clean_Discharge_Date), 
				Sum([Billing Amount]) as Yearly_Total,
				Lead(Sum([Billing Amount]),1) 
						OVER(Partition by YEAR(Clean_Discharge_Date) order by YEAR(Clean_Discharge_Date)) as Previous_YearBilling
				
		FROM Projects..Healthcare
		Group by YEAR(Clean_Discharge_Date)
		Order by YEAR(Clean_Discharge_Date);
---------------------------
	...
	----------
		...
		Select	[Admission Type], Year(Clean_Discharge_Date), Count([Admission Type])
		FROM Projects..Healthcare
		Group by [Admission Type], Year(Clean_Discharge_Date)
		Order by Year(Clean_Discharge_Date)


	--Looking for Patients that were admittd more than once
	...
	Select *, ROW_NUMBER() OVER (Partition by 
						...
						Order by Clean_Name) as Unique_Patient_Record
	FROM Projects..Healthcare
	Order by Unique_Patient_Record Desc
	...
	FROM Projects..Healthcare
	Where
		(Clean_Name = 'Amanda Smith'
		and Age = 41
		and Gender ='Female'
		and [Blood Type] = 'O+'
		and [Insurance Provider] = 'Aetna')

		OR
		(Clean_Name = 'Mary Hill'
		and Age = 37
		and Gender ='Female'
		and [Blood Type] = 'O+'
		and [Insurance Provider] = 'Medicare')
